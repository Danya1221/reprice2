...
class PriceSyncEngine:
    ...
    async def _reconcile_discovered_blocks(self, parsed_blocks):
        """
        Remove stale auto blocks from DB/channel before current price is rendered.
        """
        current_auto = {
            key
            for key, rows in (parsed_blocks or {}).items()
            if key.startswith("auto_") and rows
        }
        ...
        stale = {
            key
            for key in discovered
            if key not in current_auto
        }

        if "auto_brand_samsung" in current_auto:
            stale.add("auto_brand_galaxy")

        if "auto_brand_samsung_tablets" in current_auto:
            stale.add("auto_brand_tab")

        # Legacy junk created when wholesale tier rows were mistaken for products.
        stale.update({
            "auto_brand_от",
            "auto_brand_опт",
    ...
    async def _set_block_text(self, key, lines, closed=False):
        ...
        # No current products = no visible block.
        if key != "other" and not (lines or []):
            message_ids = dict(self.state.get("message_ids") or {})
            extra_map = dict(self.state.get("block_extra_message_ids") or {})
            delete_ids = []
            ...
            for extra_id in extras:
                try:
                    delete_ids.append(int(extra_id))
                except Exception:
                    pass
        ...
        # Supplier is OPEN but this block is absent in the fresh price.
        # Do NOT delete the message or product names.
        # Reuse the last saved price only as a source of product names,
        # then strip prices from those rows.
        if not closed and not lines:
            last_full_price = self.state.get("last_full_price", "")

            if last_full_price:
                previous = parse_full_price(last_full_price)
                previous_lines = previous["blocks"].get(key, [])
            ...
            # No historical products for this block yet: keep the block title.
            ...
            if not message_id:
                ...
                try:
                    ...
                    # Saved ID may point to a message deleted manually, deleted
                    # during "suppliers closed", or created by an older session.
                    print(
                        f"♻️ Блок {key}: старый message_id={message_id} "
                        f"нельзя редактировать ({e}); создаю заново",
                        flush=True,
                    )
                    message_ids.pop(key, None)
        ...
        # If the amount of physical messages changed, a full ordered rebuild
        # is needed after this price pass to keep the logical group contiguous.
        if old_count != len(extra_bodies) and old_count > 0:
            self._logical_layout_dirty = True
    ...
    async def apply_price(self, raw_text):
        ...
        if self._logical_layout_dirty:
            await self.rebuild_block_order()
            self._logical_layout_dirty = False
        else:
            await self.ensure_navigation(force_bottom=True)
        ...
        async with self.lock:
            ...
            try:
                ...
                if not merged_raw.strip():
                    ...
                    raise RuntimeError(
                        "Источники ответили, но обычные розничные строки с ценой не распознаны: "
                        + source_names
                    )
