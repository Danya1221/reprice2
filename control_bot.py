...
def _source_label(cfg, slot):
    if not cfg:
        return f"{slot}️⃣ не настроен"

    source_type = (cfg.get("type") or "bot").strip().lower()

    if source_type == "chat":
        return f"{slot}️⃣ 💬 {cfg.get('chat') or 'группа'}"

    return f"{slot}️⃣ 🤖 {cfg.get('bot') or 'бот'}"
...
def sources_menu(runtime):
    ...
    for slot in (2, 3):
        ...
        rows.append([
            InlineKeyboardButton(
                text=f"{_source_label(cfg, slot)} | {status}",
...
def source_slot_menu(runtime, slot):
    ...
    source_type = (cfg.get("type") or "bot").strip().lower() if cfg else "bot"
    ...
    rows = [
        ...
        [
            InlineKeyboardButton(
                text="🤖 Настроить прайс-бот",
                callback_data=f"source_config_bot:{slot}",
            )
        ],
        [
            InlineKeyboardButton(
                text="💬 Настроить группу/канал",
                callback_data=f"source_config_chat:{slot}",
            )
        ],
    ]

    # Only supplier-bot source needs its own dedicated Telegram account.
    # Group/channel source reuses already connected accounts automatically.
    if source_type != "chat":
        rows.append([
            InlineKeyboardButton(
                text="🧪 Проверить запрос сейчас",
                callback_data=f"source_test:{slot}",
            )
        ])
        rows.append([
            InlineKeyboardButton(
...
def blocks_menu(state):
    ...
    rows = [
        [
            InlineKeyboardButton(
                text=f"{'✅' if iphone_enabled else '❌'} 🍎 iPhone",
                callback_data="block:15_sim",
            )
        ],
    ]

    # Real automatically discovered brand/family blocks.
    if discovered:
        for key in state.get("block_order") or []:
            if key not in discovered:
                continue

            # Hidden visual children are part of the keeper block now.
            if key == "auto_dyson_vacuum":
                continue
...
def status_text(state, runtime=None):
    enabled = state.get("sync_enabled", True)
    ...
    if runtime is not None:
        # Source 1 = the original HI Telegram account.
        source_lines.append(
            f"1️⃣ Основной HI: <b>{auth_label}</b>"
        )
        ...
        for slot in (2, 3):
            ...
            kind = "💬 Группа" if source_type == "chat" else "🤖 Прайс-бот"
            status = runtime.extra_source_status(slot)
            action = state.get(f"source_{slot}_last_action")

            line = f"{slot}️⃣ {kind}: <b>{status}</b>"
            if action:
                line += f"\n   ↳ {action}"
    ...
    return (
        ...
        f"🏪 Основной поставщик: <b>{supplier_labels.get(supplier_status, '⚪ ещё не проверен')}</b>\n"
...
async def run_control_bot(token, admin_id, state, engine):
    ...
    async def sync_now(callback: CallbackQuery):
        ...
        try:
            ...
            for slot in (2, 3):
                action = state.get(f"source_{slot}_last_action")
                if action:
                    source_lines.append(f"{slot}️⃣ {action}")
            ...
            text = "✅ <b>Готово</b>\n\n" + status_text(state, engine) + source_diag
        except Exception as e:
    ...
    async def open_sources(callback: CallbackQuery):
        ...
        await callback.message.edit_text(
            "👥 <b>Источники прайса</b>\n\n"
            "1️⃣ Основной HI — прайс-бот.\n"
            "2️⃣ Второй прайс-бот — отдельный Telegram-аккаунт.\n"
            "3️⃣ Группа/канал — отдельный вход НЕ нужен.\n\n"
            "Группа читается через уже подключённые аккаунты автоматически.\n"
            "Все прайсы запрашиваются одновременно.",
            parse_mode="HTML",
    ...
    async def open_source_slot(callback: CallbackQuery):
        ...
        source_type = (cfg.get("type") or "bot") if cfg else "не задан"

        if source_type == "chat":
            details = (
                "Тип: <b>💬 группа/канал</b>\n"
                f"Чат: <code>{cfg.get('chat') or 'не задан'}</code>\n"
                "Режим: только чтение свежего прайса"
            )
        else:
            details = (
                "Тип: <b>🤖 прайс-бот</b>\n"
                f"Бот: <code>{(cfg.get('bot') or 'не задан') if cfg else 'не задан'}</code>\n"
                f"Запрос: <code>{(cfg.get('request') or '/prices') if cfg else '/prices'}</code>"
            )
    ...
    @dp.callback_query(F.data.startswith("source_config_bot:"))
    async def configure_bot_source(callback: CallbackQuery):
        if not allowed(callback.from_user.id):
            return await deny(callback)
        ...
        await callback.message.edit_text(
            f"🤖 <b>Прайс-бот — источник {slot}</b>\n\n"
            "Пришли одной строкой:\n"
            "<code>@supplier_bot /prices</code>\n\n"
            "Если команда другая — просто укажи её после username.",
            parse_mode="HTML",
        )
        await callback.answer()


    @dp.callback_query(F.data.startswith("source_config_chat:"))
    async def configure_chat_source(callback: CallbackQuery):
        if not allowed(callback.from_user.id):
            return await deny(callback)
        ...
        await callback.message.edit_text(
            f"💬 <b>Группа/канал — источник {slot}</b>\n\n"
            "Пришли ссылку, @username или Telegram ID группы/канала.\n\n"
            "Примеры:\n"
            "<code>@supplier_prices</code>\n"
            "<code>https://t.me/supplier_prices</code>\n"
            "<code>-1001234567890</code>\n\n"
            "Аккаунт этого источника должен иметь доступ к группе.",
            parse_mode="HTML",
        )
        await callback.answer()


    @dp.callback_query(F.data.startswith("source_test:"))
    async def test_source_now(callback: CallbackQuery):
        if not allowed(callback.from_user.id):
            return await deny(callback)
        ...
        if not cfg:
            await callback.answer("Источник не настроен.", show_alert=True)
            return

        source_type = (cfg.get("type") or "bot").strip().lower()
        if source_type == "chat":
            await callback.answer("Это группа, а не прайс-бот.", show_alert=True)
            return

        await callback.answer("Отправляю команду поставщику…")

        try:
            raw = await engine.extra_sources.request_bot_price(slot)
            if not raw:
                raise RuntimeError("пустой ответ")

            preview = raw[:900].replace("<", "&lt;").replace(">", "&gt;")
            await callback.message.answer(
                f"✅ <b>Источник {slot} ответил.</b>\\n\\n"
                f"<code>{preview}</code>",
                parse_mode="HTML",
            )
        except Exception as e:
            await callback.message.answer(
                f"❌ <b>Источник {slot} не отдал прайс.</b>\\n\\n"
                f"<code>{str(e)}</code>\\n\\n"
                f"Последнее действие: "
                f"<code>{state.get(f'source_{slot}_last_action', '—')}</code>",
                parse_mode="HTML",
            )
    ...
    async def login_source(callback: CallbackQuery):
        ...
        slot = int(callback.data.split(":")[1])
        cfg = engine.get_extra_source(slot)
        source_type = (cfg.get("type") or "bot") if cfg else "bot"
        configured = (
            bool(cfg.get("chat"))
            if source_type == "chat"
        ...
        if not cfg or not configured:
            await callback.answer(
                "Сначала настрой источник: бот или группу.",
                show_alert=True,
            )
            return
    ...
    async def custom_value_message(message: Message):
        ...
        if extra:
            ...
            if mode == "config_chat":
                ...
                if not chat_ref:
                    await message.answer("Пришли ссылку, @username или ID группы.")
                    return
                ...
                await message.answer(
                    f"✅ Группа/канал для источника {slot} сохранена:\n"
                    f"<code>{chat_ref}</code>\n\n"
                    "Бот ничего туда не отправляет — только читает свежий прайс.\n"
                    "Отдельный вход для группы не нужен.",
                    parse_mode="HTML",
                    reply_markup=source_slot_menu(engine, slot),
                )
                return
