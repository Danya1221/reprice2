...
from telethon.errors import AuthKeyDuplicatedError, AuthKeyUnregisteredError, SessionRevokedError, SessionPasswordNeededError
...
from multi_source import ExtraSourceManager
...
class TelegramRuntime:
    ...
    def __init__(self, state, database):
        ...
        self._closing = False
        self.extra_sources = ExtraSourceManager(state, database)
    ...
    async def start(self):
        ...
        # Restore optional supplier accounts 2/3 from PostgreSQL.
        with suppress(Exception):
            await self.extra_sources.start()
    ...
    async def _activate(self):
        ...
        # Group/channel price source reuses this authorized account automatically.
        self.extra_sources.set_primary_client(self.client)
        ...
        print(
            "🚀 Telegram runtime готов: запросы поставщикам больше не ждут "
            "построения структуры канала",
            flush=True,
        )
    ...
    async def handle_auth_failure(self, error=None):
        async with self.auth_lock:
            ...
            if isinstance(error, (AuthKeyDuplicatedError, AuthKeyUnregisteredError, SessionRevokedError)):
                self.database.delete(self.SESSION_DB_KEY)
                self._clear_pending_login()
    ...
    def get_extra_source(self, slot):
        return self.extra_sources.get_source(slot)

    def set_extra_source(self, slot, **values):
        return self.extra_sources.set_source(slot, **values)

    def remove_extra_source(self, slot):
        return self.extra_sources.remove_source(slot)

    def extra_source_status(self, slot):
        return self.extra_sources.source_status(slot)

    async def begin_extra_login(self, slot, phone):
        return await self.extra_sources.begin_login(slot, phone)

    async def submit_extra_code(self, slot, code):
        return await self.extra_sources.submit_code(slot, code)

    async def submit_extra_2fa(self, slot, password):
        return await self.extra_sources.submit_2fa(slot, password)
    ...
    async def close(self):
        ...
        with suppress(Exception):
            await self.extra_sources.close()
