...
class PriceSyncEngine:
    ...
    async def _prepare_publish_structure(self):
        async with self._publish_init_lock:
            if self._publish_ready.is_set():
                return

            try:
                print("🧱 Готовлю структуру канала в фоне…", flush=True)
                ...
                print("✅ Структура канала готова", flush=True)

            except Exception as e:
                self._publish_init_error = e
                print(f"❌ Ошибка подготовки структуры канала: {e}", flush=True)
                raise
    ...
    async def _wait_publish_ready(self):
        ...
        if self._publish_init_task is None or self._publish_init_task.done():
            self._publish_init_task = asyncio.create_task(
                self._prepare_publish_structure()
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Запросы поставщикам отправлены, но канал не готов к публикации: {e}"
            ) from e


    async def start(self):
        # Register supplier edit handler immediately.
        self.client.add_event_handler(
            self.on_supplier_edit,
            events.MessageEdited(chats=SUPPLIER_BOT),
        )

        # IMPORTANT: do not await this here.
        # Runtime becomes ready immediately and supplier requests can be sent now.
        if self._publish_init_task is None or self._publish_init_task.done():
            self._publish_init_task = asyncio.create_task(
                self._prepare_publish_structure()
            )
