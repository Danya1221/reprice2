...
def detect_dynamic_brand_subclass(block_key: str, line: str) -> str:
    ...
    if block_key == "auto_brand_samsung":
        # One Samsung block with broad series headings.
        # Exact model stays on each product row (A56, S26 Ultra, Fold7, etc.).
        ...
        if re.match(r"^(?:Tab|Galaxy\s+Tab)\b", clean, re.I):
            return "Galaxy Tab"

        if re.match(r"^(?:Watch|Galaxy\s+Watch)\b", clean, re.I):
            return "Galaxy Watch"

        if re.match(r"^(?:Buds|Galaxy\s+Buds)\b", clean, re.I):
            return "Galaxy Buds"
