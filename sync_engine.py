...
from parser import BLOCK_KEYS, BLOCK_TITLES, add_markup_to_line, closed_match, parse_full_price
...
class PriceSyncEngine:
    ...
    async def start(self):
        # Убираем старую текстовую навигацию от предыдущих версий.
        await self._delete_navigation_messages()

        await self.ensure_structure()

        # Навигация должна быть последним сообщением и иметь inline-кнопки.
    ...
    async def ensure_navigation(self, force_bottom=False):
        """
        При обычном обновлении редактирует навигацию.
        Если force_bottom=True — удаляет её и создаёт заново последним сообщением.
        """
        if not self.bot:
            print(
                "⚠️ CONTROL_BOT_TOKEN не указан — навигация пропущена",
                flush=True,
            )
            return
        ...
        try:
            ...
            print(
                f"🧭 Навигация создана последней; кнопок: {button_count}",
                flush=True,
            )

        except Exception as e:
            print(
                "⚠️ Не удалось создать навигацию с кнопками. "
                "Проверь, что управляющий бот является админом TARGET_CHANNEL. "
                f"Ошибка: {e}",
                flush=True,
            )
