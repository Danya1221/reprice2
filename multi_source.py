...
class ExtraSourceManager:
    ...
    async def read_chat_price(self, slot):
        ...
        Nothing is sent to the chat. We find the newest message containing
        product-price rows and combine price-bearing messages from the same
        15-minute supplier posting batch.
        """
        slot = int(slot)
        cfg = self.get_source(slot)
        ...
        async with self._locks[slot]:
            ...
            try:
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"не могу открыть группу/канал {chat_ref_raw}: {e}"
                    ) from e
                ...
                if not result:
                    raise RuntimeError(f"прайс в {chat_ref_raw} пустой")
    ...
    async def collect_extra_prices(self):
        ...
        for slot in (2, 3):
            ...
            try:
                ...
                errors.append(f"Источник {slot}: {e}")
