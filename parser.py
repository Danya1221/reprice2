...
def detect_dynamic_brand_subclass(block_key: str, line: str) -> str:
    ...
    if block_key == "auto_brand_samsung":
        if low.startswith("samsung "):
            # Samsung-branded non-phone products/accessories.
            title = _cut_model_tail(txt)
            return title or "Samsung — аксессуары"

        clean = re.sub(r"^galaxy\s+", "", txt, flags=re.I)
        clean = norm(clean.replace(".", " "))

        patterns = (
            (r"^(A\d{2})\b", lambda m: f"Galaxy {m.group(1).upper()}"),
            (r"^(S\d{2})\s+FE\b", lambda m: f"Galaxy {m.group(1).upper()} FE"),
            (r"^(S\d{2})\s+Edge\b", lambda m: f"Galaxy {m.group(1).upper()} Edge"),
            (r"^(S\d{2})\s+Ultra\b", lambda m: f"Galaxy {m.group(1).upper()} Ultra"),
            (r"^(S\d{2})\b", lambda m: f"Galaxy {m.group(1).upper()}"),
            (r"^(Z\s+Fold\d+)\b", lambda m: "Galaxy " + norm(m.group(1))),
            (r"^(Z\s+Flip\d+)\b", lambda m: "Galaxy " + norm(m.group(1))),
        )
