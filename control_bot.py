...
async def run_control_bot(token, admin_id, state, engine):
    ...
    async def open_sources(callback: CallbackQuery):
        ...
        await callback.message.edit_text(
            ...
            "2️⃣/3️⃣ Можно сделать прайс-ботом или обычной группой/каналом.\n\n"
            "Из группы бот только читает свежий прайс — ничего туда не отправляет.\n"
            "Дубли объединяются по минимальной закупке.",
            parse_mode="HTML",
