...
from parser import PRICE_END_RE, closed_match, is_wholesale_tier_line
...
def _canonical_product_key(line):
    ...
    # Normalize Samsung naming across suppliers so duplicates collapse:
    # Samsung Galaxy S26 == Galaxy S26 == Samsung S26.
    text = re.sub(
    ...
    text = re.sub(
        r"^samsung\s+(?=(?:a\d{2}|s\d{2}|z\s*(?:fold|flip)\d+|tab\s+))",
        "galaxy ",
        text,
        flags=re.I,
    )
...
RETAIL_LABEL_RE = re.compile(
    r"(?:обычная\s+цена|розничная\s+цена|розница|ррц|наша\s+цена|цена|retail)",
    re.I,
)

WHOLESALE_START_RE = re.compile(
    r"(?:"
    r"от\s*\d+\s*(?:шт|штук)\b"
    r"|\b\d+\s*(?:шт|штук)\s*(?:\+|и\s+более)?"
    r"|мелк(?:ий|ого)\s+опт"
    r"|крупн(?:ый|ого)\s+опт"
...
def _rstrip_separators(text):
    return (text or "").rstrip(" \t|/;,:–—-")


def _cut_wholesale_tail(line):
    """
    Keep only the normal-price portion before the first wholesale tier.

    Product — 68300 | От 1 шт — 67500
    -> Product — 68300
    """
    text = _clean_spaces(line)
    m = WHOLESALE_START_RE.search(text)

    if not m:
        return text

    return _rstrip_separators(text[:m.start()])


def _terminal_price(line):
    text = _clean_spaces(line)
    m = PRICE_END_RE.search(text)
...
def _strip_retail_label_from_product(text):
    """
    Galaxy S26 Black | Цена -> Galaxy S26 Black
    """
    value = _rstrip_separators(_clean_spaces(text))
    ...
    return _rstrip_separators(value)


def _normalize_retail_row(product_text, price):
    product = _strip_retail_label_from_product(product_text)
    product = _clean_spaces(product)

    if not product or price is None:
        return None

    return f"{product} — {int(price)}"


def _looks_like_product_title(line):
    text = _clean_spaces(line)

    if not text or is_wholesale_tier_line(text):
        return False

    # Standalone price labels are not product titles.
    stripped = _rstrip_separators(text)
    if RETAIL_LABEL_RE.fullmatch(stripped):
        return False

    low = stripped.casefold()
    pure_headers = {
        "прайс", "прайс лист", "актуальный прайс", "наличие",
        "в наличии", "розница", "цена", "цены", "ррц",
        "смартфоны", "планшеты", "наушники", "аксессуары",
        "apple", "samsung", "xiaomi", "dyson",
    }
    if low in pure_headers:
        return False
    ...
    has_model_signal = bool(
        re.search(r"\d", text)
        or re.search(
            r"\b(?:airpods|airwrap|watch|buds|pixel|quest|switch|macbook|marshall|bose)\b",
            text,
            re.I,
        )
    )

    return has_letters and has_model_signal and len(text) >= 5


APPLE_FAMILY_HEADER_RE = re.compile(
    r"^(?:"
    r"macbook(?:\s+(?:pro|air))?"
    r"|mac\s+(?:mini|studio|pro)"
    r"|imac"
    r"|ipad(?:\s+(?:pro|air|mini))?"
    r"|apple\s+watch(?:\s+ultra)?"
    r"|watch(?:\s+ultra)?"
    r"|airpods(?:\s+(?:pro|max))?"
    r"|homepod(?:\s+mini)?"
    r")\b",
...
OTHER_FAMILY_HEADER_RE = re.compile(
    r"^(?:"
    r"samsung\s+galaxy\s+[asz]\w*"
    r"|galaxy\s+[asz]\w*"
    r"|dyson\s+\w+"
    r"|dji\s+\w+"
    r"|xiaomi\s+\w+"
...
EXPLICIT_PRODUCT_PREFIX_RE = re.compile(
    r"^(?:"
    r"iphone\b|samsung\b|galaxy\b|macbook\b|mac\s+(?:mini|studio|pro)\b"
    r"|imac\b|ipad\b|apple\s+watch\b|watch\b|airpods\b|homepod\b"
    r"|apple\s+tv\b|beats\b|dyson\b|dji\b|xiaomi\b|redmi\b|poco\b"
    r"|huawei\b|google\b|pixel\b|marshall\b|bose\b|harman\b|jbl\b"
    r"|ray-?ban\b|meta\b|oakley\b|nintendo\b|kodak\b|fujifilm\b"
    r")",
    re.I,
)


def _is_family_context_header(text):
    value = _clean_spaces(text)
    if not value:
        return False

    # Context header must stay short; a full configured product should not be
    # prepended to the next row.
    if len(value.split()) > 5:
        return False

    return bool(
        APPLE_FAMILY_HEADER_RE.search(value)
        or OTHER_FAMILY_HEADER_RE.search(value)
    )


def _has_explicit_product_prefix(text):
    return bool(EXPLICIT_PRODUCT_PREFIX_RE.search(_clean_spaces(text)))


def _join_context_variant(context, variant):
    context = _clean_spaces(context)
    variant = _clean_spaces(variant)
    ...
    # Avoid "MacBook Pro MacBook Pro..."
    if variant.casefold().startswith(context.casefold()):
        return variant

    return f"{context} {variant}"


def _extract_retail_rows(raw):
    """
    Extract ONLY the ordinary/direct retail price.

    Supported layouts:

    Product — 68300 | От 1 шт — 67500

    Product | Цена: 68300 | От 1 шт — 67500
    ...
    Wholesale tiers are never emitted.
    """
    result = []
    pending_product = None

    for raw_line in (raw or "").splitlines():
        original = _clean_spaces(raw_line)
        if not original:
            continue

        retail_part = _cut_wholesale_tail(original)
        if not retail_part:
            continue

        price = _terminal_price(retail_part)

        if price is not None:
            m = PRICE_END_RE.search(retail_part)
            before_price = retail_part[:m.start("prefix")].strip()
            before_clean = _rstrip_separators(_clean_spaces(before_price))

            # Metadata-only price line: "Цена: 68300", "Розница — 68300".
            label_without_prefix = _strip_retail_label_from_product(before_clean)
            label_only = bool(
                RETAIL_LABEL_RE.search(before_clean)
                and not label_without_prefix
            )

            # Bare price line after a pending product title.
            bare_price = not bool(re.search(r"[A-Za-zА-Яа-я]", before_clean))

            if pending_product and (label_only or bare_price):
                row = _normalize_retail_row(pending_product, price)
                if row:
                    result.append(row)
                continue

            # Direct product + retail price on same line.
            product_text = _strip_retail_label_from_product(before_price)

            if _looks_like_product_title(product_text):
                # Supplier often sends:
                #   MacBook Pro
                #   14 M4 16/512 Space Black — 150000
                # Without the section context, "14 M4..." has no usable brand.
                if (
                    pending_product
                    and _is_family_context_header(pending_product)
                    and not _has_explicit_product_prefix(product_text)
                ):
                    product_text = _join_context_variant(
                        pending_product,
                        product_text,
                    )

                row = _normalize_retail_row(product_text, price)
                if row:
                    result.append(row)

                    # A full variant must not replace the short family header,
                    # otherwise only the first row in a section gets context.
                    if not (
                        pending_product
                        and _is_family_context_header(pending_product)
                    ):
                        pending_product = product_text
                continue

            # Do not invent products from "Цена", "Розница", etc.
            continue

        # Product/family title without price; remember for following rows.
        if _looks_like_product_title(retail_part):
            pending_product = retail_part
...
def merge_supplier_prices(raw_prices):
    """
    Merge only ordinary retail prices.

    Any wholesale tier ("От N шт", opt, wholesale) is ignored completely.
    """
    best = {}

    for source_label, raw in raw_prices:
        for line in _extract_retail_rows(raw):
...
def _count_price_rows(text):
    return len(_extract_retail_rows(text))
    ...
    async def request_bot_price(self, slot):
        ...
        async with self._locks[slot]:
            ...
            try:
                ...
                result = "\n".join(parts)
                recognized_rows = sum(
                    _count_price_rows(part)
                    for part in parts
                )
                self.state.set(
                    f"source_{slot}_last_action",
                    (
                        f"✅ {bot_name}: прайс получен "
                        f"({len(parts)} сообщ., {recognized_rows} товарных строк)"
                    ),
                )
                return result
        ...
        async with self._locks[slot]:
            ...
            for client in candidates:
                try:
                    ...
                    if result:
                        recognized_rows = _count_price_rows(result)
                        self.state.set(
                            f"source_{slot}_last_action",
                            (
                                f"✅ группа прочитана "
                                f"({len(selected)} сообщ., "
                                f"{recognized_rows} товарных строк)"
                            ),
                        )
                        return result
