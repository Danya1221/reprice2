...
class TelegramRuntime:
    ...
    async def start(self):
        ...
        try:
            ...
            # Dead auth keys cannot be revived. Remove them from PostgreSQL so
            # Railway never loops forever on the same broken session.
            print(f"❌ Telegram session недействительна: {e}", flush=True)
            ...
            self.state.update(
                ...
                supplier1_status="error",
                supplier2_status="error",
    ...
    async def _activate(self):
        ...
        self.state.update(
            ...
            last_result="Telegram подключен. Два прайса готовы к синхронизации.",
        )

        print(f"✅ Telethon: {me.username or me.id}", flush=True)
        print(f"📦 Источник 1: {SUPPLIER_BOT}", flush=True)
        print(f"📦 Источник 2: {SUPPLIER_BOT_2}", flush=True)
        print(f"➡️ Канал: {TARGET_CHANNEL}", flush=True)
