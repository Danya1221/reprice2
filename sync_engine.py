...
class PriceSyncEngine:
    ...
    async def set_block_enabled(self, key: str, enabled: bool):
        ...
        if full_price and not self.state.get("supplier_closed", False):
            parsed = parse_full_price(full_price)
    ...
    async def set_closed(self):
        """
        Поставщик закрыт:
        - включённые блоки остаются;
        - все цены/позиции из них удаляются;
        - выключенные блоки не создаются;
        - снизу одно сообщение "Продажи временно закрыты".
        """
        blocks_enabled = self.state.get("blocks") or {}
        ...
        self.state.update(
            ...
            last_result="продажи закрыты — цены очищены",
        )

        print(
            "🚫 Поставщик закрыт: цены очищены из включённых блоков",
            flush=True,
        )
