    ...
    async def sync_once(self, forced=False):
        async with self.lock:
            ...
            try:
                ...
                # Slot 1: existing HI account/supplier.
                # IMPORTANT: one broken supplier must never stop the others.
                try:
                    primary_raw = await self.request_price()
                    if primary_raw and not closed_match(primary_raw, CLOSED_TEXT):
                        raw_prices.append(("Аккаунт 1", primary_raw))
                    elif primary_raw:
                        source_errors.append("Аккаунт 1: поставщик закрыт")
                except Exception as e:
                    source_errors.append(f"Аккаунт 1: {e}")

                # Slots 2/3: optional additional Telegram accounts.
                # ExtraSourceManager already isolates failures per slot.
                if self.extra_sources is not None:
                    extra_prices, extra_errors = await self.extra_sources.collect_extra_prices()
