...
from parser import PRICE_END_RE, closed_match, is_wholesale_tier_line, is_retail_product_price_line
...
def merge_supplier_prices(raw_prices):
    ...
    for source_label, raw in raw_prices:
        for raw_line in (raw or "").splitlines():
            ...
            # Retail-only: wholesale tiers never enter dedup/comparison.
            # If a normal direct product price exists, that is the only candidate.
            if not is_retail_product_price_line(line):
                continue
...
def _count_price_rows(text):
    return sum(
        ...
        if is_retail_product_price_line(_clean_spaces(line))
    )
